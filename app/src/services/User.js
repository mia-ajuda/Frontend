...
class UserService {
  ...
  async logIn(data) {
    try {
      await firebaseAuth.auth().signInWithEmailAndPassword(data.email, data.password);
  ...
  async logInWithFacebook(navigation) {
    try {
      ...
      if (type === 'success') {
        ...
        const isExists = await api.get(`/checkUserExistence/${userFacebookInfo.profile.email}`);
  ...
  async loginInWithGoogle(navigation) {
    try {
      ...
      if (googleResponse.type === 'success') {
        ...
        const isExists = await api.get(`/checkUserExistence/${googleResponse.user.email}`);
