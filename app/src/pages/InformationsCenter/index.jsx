...
export default function InformationsCenter() {
    ...
    return (
        <View className='flex-1 w-screen bg-new_background p-4'>
            <SliderModal visible={modalVisible} pages={pages} closeModal={() => setModalVisible(false)} />
            <EmergencyNumbers setVisible={setEmergencyNumbersVisible} visible={emergencyNumbersVisible} />
            {faqOption.map((faq, i) => (
                <TutorialCard
                    key={faq.title}
                    description={faq.description}
                    title={faq.title} margin={'mb-2'}
                    leftAligment={faq.id % 2}
                    onPress={() => { faq.emergencyModal ? setEmergencyNumbersVisible(true) : handleShowModal(i) }} />
            ))}
        </View>
    );
}
