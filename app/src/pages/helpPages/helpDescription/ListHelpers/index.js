...
export default function ListHelpers({ clickAction, stateAction, helpId, navigation }) {
  ...
  const helperPhoto = (photo) =>
    photo.includes('http') ? { uri: photo } : { uri: `data:image/png;base64,${photo}` };
  ...
  return (
    <View
      style={[
        styles.container,
        stateAction ? { justifyContent: 'flex-start' } : { justifyContent: 'flex-end' },
      ]}>
      {loading ? (
          ...
          <View>
            ...
            <View style={styles.volunteerContainer}>
              <View style={{ flexDirection: 'row' }}>
                <Image style={styles.volunteerImage} source={helperPhoto(helperImage)} />
                <View style={{ width: '80%' }}>
                  <Text style={[{ fontFamily: 'montserrat-semibold' }]}>{helperName}</Text>
              ...
              {help.status === 'on_going' || help.status === 'helper_finished' ? (
                <Button press={() => openModal('finish')} title="Finalizar Ajuda" large />
              ) : (
                <></>
              )}
            </View>
        ...
        <TouchableOpacity style={styles.buttonHelpers} onPress={() => clickAction(!stateAction)}>
            ...
            {possibleHelpers.map((helper) => (
              <TouchableOpacity
                ...
                <View style={styles.helper}>
                  <Image style={styles.imageProfile} source={helperPhoto(helper.photo)} />
                  <View>
