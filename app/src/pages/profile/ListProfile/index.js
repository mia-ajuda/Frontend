...
import { View, ScrollView, Text, ImageBackground, TouchableOpacity, Alert } from 'react-native';
...
export default function Profile({ navigation }) {
  ...
  function formatPhone(phone) {
    if (phone.length === 13) {
      return `(${phone.slice(3, 5)}) ${phone.slice(5, 9)}-${phone.slice(9, 14)}`;
    }

    return `(${phone.slice(3, 5)}) ${phone.slice(5, 10)}-${phone.slice(10, 14)}`;
  }
  ...
  const sendPhoto = async () => {
    try {
      ...
      Alert.alert('Sucesso', 'Foto atualizada com sucesso!', [{ text: 'OK', onPress: () => {} }], {
        cancelable: false,
      });
    } catch (err) {
      setLoadingModal(false);
  ...
  function parseDate(date) {
    ...
    return `${('0' + (newDate.getDate() + 1)).slice(-2)}/${('0' + (newDate.getMonth() + 1)).slice(
      -2,
    )}/${newDate.getFullYear()}`;
  }
